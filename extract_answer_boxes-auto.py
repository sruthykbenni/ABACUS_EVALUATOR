...
def _detect_page_quad(img):
    ...
    try:
        ...
        if color_quad is not None:
            # validate color_quad using edge support
            min_support, _ = _quad_edge_support(edges, color_quad)
            if _valid_page_quad(color_quad, float(gray.shape[0] * gray.shape[1]), float(orig_h * orig_w)) and min_support >= PERSPECTIVE_MIN_EDGE_SUPPORT:
                return color_quad
    except Exception:
        pass
...
def _answer_region_box(x0, x1, fraction):
    width = max(x1 - x0, 1)
    answer_w = max(1, int(round(width * fraction)))
    return max(x0, x1 - answer_w), x1
...
def _crop_row_layout(img, page, candidate, csv_rows, global_state):
    ...
    for r_idx, (y0, y1) in enumerate(sorted(candidate.bands, key=lambda band: band[0])):
        ...
        for c_idx in range(len(lines) - 1):
            ...
            ans_x0p, ans_x1p = _answer_region_box(x0p, x1p, ANSWER_REGION_FRACTION)
            crop = img[y0p:y1p, ans_x0p:ans_x1p]
            ...
            cv2.rectangle(debug, (ans_x0p, y0p), (ans_x1p, y1p), (0, 255, 0), 1)
